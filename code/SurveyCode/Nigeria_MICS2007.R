...
df<-df%>%mutate(
  ...
  unmet_need=ifelse(as.character(UnmetSim)=="MissingData", NA, unmet_need),
...
df$state[df$state == "abuja fct"] <- "fct-abuja"
df$state[df$state=="akwa-ibom"] <- "akwa ibom"
df$state[df$state=="cross-rivers"] <- "cross river"
df$state[df$state=="plataeu"] <- "plateau"
...
final <- compute_survey_proportions(dhs=df)
final$year<-2007
final$recode<-"MICS3"
...
write_csv(as.data.frame(final),
          "Data/Processed/Nigeria/Processed_Nigeria2007.csv")
