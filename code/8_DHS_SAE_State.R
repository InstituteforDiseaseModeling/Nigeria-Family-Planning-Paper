...
##
##
## Fitting the model 2 on the DHS data at state level
...
All<-All%>%mutate(logit_mcpr=ifelse(!is.na(modern_method),logit(modern_method),NA),
                  var_logit_mcpr=(se.modern_method^2)/(modern_method^2*(1-modern_method)^2))

All$logit_mcpr[All$se.modern_method<0.001]<-NA
All$var_logit_mcpr[All$se.modern_method<0.001]<-NA
...
grid<-expand.grid(row_num=key2$row_num,year=c(1990:2018))
grid<-grid%>%left_join(key2)
dim(grid)
dim(All)
...
plot(plotvar2[order_p]*100,1:37,xlim=c(0,30),pch=19,
...
text(x=-5,y = 1:37+0.75, par("usr")[3]-0.015 , labels = StateNames, 
     srt = 0, pos = 1, xpd = TRUE)


segments(All$up[All$year==2013][order_p]*100,1:37,
         All$low[All$year==2013][order_p]*100,1:37)
# abline(v=0,lty=2)
