...
Unmet <- function(ir.data, SurveyID){
  ir.data$unmet <- NA
  ...
  ## b3_01 = Date of last birth (CMC)
  ## v008 = Date of Interview (CMC)
  ## DHS April Update: tsinceb now calculated using v222 which is based on century day codes in DHS7
  ir.data$tsinceb <- NA
  ir.data$tsinceb[which(ir.data$v000 < 7)] <- ir.data$v008[which(ir.data$v000 < 7)] - ir.data$b3_01[which(ir.data$v000 < 7)]
  ...
  ### 0 - No/Unsure, 1 - Yes
  ## m6_1 = Duration of PPA after Birth of last child in month
  ### 96 = Period Not Returned, 98 Don't know
  ir.data$pregPPA <- NA
  ir.data$pregPPA[which(ir.data$v213 == 1 | ir.data$m6_1 == 96)] <- 1
  
  #For women with 
  ##missing ir.data or "Period not Returned" as date of last menstrual
  ir.data$pregPPA[(is.na(ir.data$m6_1) | ir.data$m6_1==97 | ir.data$m6_1==99) & (ir.data$tsincep > ir.data$tsinceb)
  ...
  ir.data$pregPPA[(is.na(ir.data$m6_1) | ir.data$m6_1==97 | ir.data$m6_1==99) & ir.data$v215==995
  ...
  ir.data$wantedlast[indexWL] <- ir.data$m10_1[indexWL]
  rm(indexWL)
  ...
  ir.data$unmet_need <- NA
  ir.data$unmet_need[which(ir.data$unmet==1 | ir.data$unmet==2)]<-1
  ir.data$unmet_need[which(!ir.data$unmet %in% c(1,2))] <- 0
  
  ir.data$unmet_need_space <- as.integer(ir.data$specific_unmet==1)
  ir.data$unmet_need_limit <- as.integer(ir.data$specific_unmet==2)
  ...
  if(SurveyID=="ke70") {
    for(i in c("unmet","unmet_need","specific_unmet")){
